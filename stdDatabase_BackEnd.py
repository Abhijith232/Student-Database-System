...
def searchData(StdID="",Firstname="",Surname="",DoB="",Age="",Gender="",Address="",Mobile=""):
    con=sqlite3.connect('student.db')
    cur=con.cursor()
    cur.execute("SELECT * FROM student WHERE StdID=? OR Firstname=? OR Age=? OR Gender=? OR Address=? OR Mobile=? ",\
                (StdID,Firstname,Surname,DoB,Age,Gender,Address,Mobile))
    rows=cur.fetchall()
    con.close()
    return rows

def dataUpadate(id,StdID="",Firstname="",Surname="",DoB="",Age="",Gender="",Adress="",Mobile=""):
    con=sqlite3.connect('student.db')
    cur=con.cursor()
    cur.execute("UPDATE student SET StdID=? Firstname=?,Surname=?,DoB=?,Age=?,Gender=?,Address=?\
        Mobile=?,WHERE id=?",(StdID,Firstname,Surname,DoB,Age,Gender,Address,Mobile))
    con.commit()
    con.close()
