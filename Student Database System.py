...
class Student:
    def __init__(self,root):
        ...
        def DisplayData():
            studentlist.delete(0,END)
            for rows in stdDatabase_BackEnd.viewData():
        ...
        def searchDatabase():
            studentlist.delete(0,END)
            for row in stdDatabase_BackEnd.searchData(StdID.get(),Firstname.get(),Surname.get(),DoB.get(),Age.get(),Gender.get(),\
                                     Address.get(),Mobile.get()):
                studentlist.insert(END,rows,str(""))
        ...
        DataFrameRIGHT=LabelFrame(DataFrame,bd=1,width=450,height=600,padx=31,pady=3,relief=RIDGE,bg='Ghost White',font=('arial',20,'bold'),text='Student Details\n')
